...
- `relation` — exactly one of:
  - `serves_intent` — the change is part of what the PR title/description
    says it fixed or did.
  - `unrelated` — the change has no visible connection to the title or
    description. Worth recording even post-merge: it is the audit trail for
    scope creep that already shipped.
